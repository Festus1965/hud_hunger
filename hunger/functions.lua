...
--	by Thomas - new version, via privs
	if minetest.get_player_privs(name).kid or minetest.get_player_privs(name).sarasas 
		or minetest.get_player_privs(name).lungma or minetest.get_player_privs(name).parent
		or minetest.get_player_privs(name).friend or minetest.get_player_privs(name).donate 
		or minetest.get_player_privs(name).xanadu then
	lvl = 20
	end
	
	if minetest.get_player_privs(name).food and hunger.players[name].lvl < 10 then
	lvl = 10
	end


	if lvl < 2 then -- Verhungern vermeiden, aber am Leben lassen bei afk
		lvl = 2
	end
