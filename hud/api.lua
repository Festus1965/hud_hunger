...
function hud.change_item(player, name, def)
	if not player or not player:is_player() or not name or not def then
		...
		-- include by Thomas Wiegand -- works (!!! channel must exist)
		-- should be check if mod beerchat exist !! -- worked similar in border to show name, ip
	minetest.log("error", "[mod] hud : param: " .. dump(player ~= nil) .. " : " .. dump(player:is_player()) .. " : " .. dump(name) .. " : ")


		beerchat.send_on_channel("hud", "conip", "param: " .. dump(player ~= nil) .. " : " .. dump(player:is_player()) .. " : " .. dump(name) .. " : " )
